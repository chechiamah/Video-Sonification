...
const colorToNote = (hex) => {
  ...
  colorNoteMap.forEach((item) => {
    ...
    const distance = colorDistance(rgb.r, rgb.g, rgb.b, rangeMidpoint.r, rangeMidpoint.g, rangeMidpoint.b);
...
const getColorPosition = (frameData, hex) => {
  ...
  for (let y = 0; y < height; y += 5) {
    for (let x = 0; x < width; x += 5) {
      ...
      if (Math.abs(r - rgb.r) < 30 && Math.abs(g - rgb.g) < 30 && Math.abs(b - rgb.b) < 30) {
      ...
      await new Promise((res) => {
        video.onseeked = () => {
          ...
          prominentColors.forEach((colorObj) => {
            ...
            polySynth.triggerAttackRelease(note, duration, Tone.now() + i * frameDuration, volume / 100);
          });
